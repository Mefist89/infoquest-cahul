...
export function HowItWorks({ lang }: { lang: Lang }) {
  ...
  return <section id="how" className="home-panel scroll-mt-24"><h2 className="home-title">{t.howTitle}</h2><div className="relative mt-6 grid gap-5 md:grid-cols-3"><div className="absolute left-[16%] right-[16%] top-14 hidden border-t-2 border-dashed border-neon/45 md:block" aria-hidden="true" />{t.how.map(([title, text], index) => { const Icon = howIcons[index]; return <article key={title} className="relative z-10 flex min-h-64 flex-col items-center rounded-3xl border border-neon/30 bg-slate-950/55 px-6 py-6 text-center"><span className="mx-auto grid size-16 shrink-0 place-items-center rounded-2xl border border-neon/35 bg-neon/10 text-neon"><Icon className="size-8" aria-hidden="true" /></span><span className="mx-auto mt-4 grid size-8 shrink-0 place-items-center rounded-full border border-neon bg-background font-display text-sm text-neon">{index + 1}</span><h3 className="mt-4 text-base font-black leading-snug">{title}</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{text}</p></article>; })}</div></section>;
}
