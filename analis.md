...
| P1.8 AI multipart | Исправлено в коде 13 августа 2026 | До внешнего AI проверяются Content-Length, 4 МиБ, сигнатура, метаданные и длительность 0,5–60 с; WAV проходит RMS-порог, все форматы — проверку пустой и максимум 6000 символов расшифровки |
| P1.9 Quality gate | Частично | Локально lint, TypeScript, 38 unit/contract-тестов и build проходят; 3 публичных Playwright smoke-теста найдены. Последний GitHub Actions красный из-за рассинхронизации lock-файла при `npm ci`; авторизованные E2E требуют `E2E_AUTH_STATE_B64` |
...
| Главная страница по новому макету | Исправлено 12 августа 2026 | Добавлены hero с двумя CTA, верхняя навигация, три шага расследования, фильтры восьми дел, публичное мини-задание, слайдер, шесть навыков, реальный прогресс и бейджи, три аудитории, Chrono AI, материалы и FAQ; RU/RO и адаптивность сохранены |
| Навигация главной страницы | Исправлено 13 августа 2026 | XP и счётчик бейджей убраны из шапки; ссылки верхнего меню центрированы независимо от логотипа, профиля и переключателя языка. Прогресс и награды остаются в содержательной части страницы и профиле |
| Блок «Как проходит расследование» | Исправлено 13 августа 2026 | Ширина контейнера выровнена со слайдером «Дело: Тень» — 72 rem; карточки получили увеличенную высоту и внутренние отступы, номер этапа не перекрывает заголовок |
| Слайдер «Дело: Тень» | Исправлено 13 августа 2026 | Между мини-заданием и слайдером добавлен заметный вертикальный отступ; ширина контейнера дел выровнена с блоком «Проверь свою внимательность» — 72 rem |
| Режимы Chrono AI | Исправлено 13 августа 2026 | Добавлен центральный переключатель «Анализатор / Чат». Режимы имеют отдельные истории и черновики; анализатор сохраняет текст/аудио и структурированный риск, чат передаёт ограниченный контекст последних сообщений и не смешивается с анализами |
| UX анализатора Chrono | Исправлено 13 августа 2026 | Добавлены пять быстрых сценариев, вкладки текста/аудио, таймер записи до 60 секунд, явная остановка, плеер перед отправкой, форматы/размер файла и живой остаток дневной и аудиоквоты. Read-only RPC применена к live Supabase и возвращает только собственные лимиты пользователя |
| Ожидание аудиоанализа Chrono | Исправлено 13 августа 2026 | Вместо одного сообщения показаны четыре последовательных этапа: загрузка, распознавание речи, проверка признаков и подготовка действий. Текущий этап объявляется через `aria-live`, завершённые визуально отмечаются галочкой |
| Согласие Chrono | Исправлено 13 августа 2026 | Обязательный checkbox сохранён, длинное описание свёрнуто в доступный `details`, рядом показаны краткое предупреждение, ссылка на политику и точное уведомление: InfoQuest не сохраняет историю проверок после закрытия страницы; правила внешнего провайдера раскрыты отдельно |
| Мобильный Chrono и новая проверка | Исправлено 13 августа 2026 | На телефоне боковой персонаж заменён компактным аватаром возле заголовка, чат получил ограниченную viewport-высоту и закреплённую форму, большая реакция показывается после результата. Всегда подписанная «Новая проверка» очищает текущий диалог/черновик, отменяет клиентский запрос и озвучивание, останавливает запись, удаляет аудио и возвращает нейтральную реакцию |
| Доступность контролов и результата Chrono | Исправлено 13 августа 2026 | Отправка текста/аудио, микрофон и звук получили явные локализованные `aria-label`. `aria-live` снят со всего журнала; отдельный атомарный status объявляет только новый результат анализа: готовность, риск, вердикт и резюме |
| Категории дел на главной | Исправлено 13 августа 2026 | Четыре укрупнённых фильтра заменены восемью кнопками из единого каталога: звонки, ссылки, аккаунты, предложения, дипфейки, переводы, слухи и тролли. Каждая кнопка выделяет одно соответствующее дело и имеет RU/RO-подпись |
...
| Главная страница | Полноценный публичный игровой лендинг | Новый экран соответствует утверждённому макету; щит, XP и бейджи связаны с Supabase, а основной CTA вошедшего пользователя ведёт в профиль |
...
### P1.8. Таймаут и защита audio multipart добавлены
...
До `request.formData()` проверяется заявленный размер HTTP-запроса; сам разбор формы защищён обработкой ошибки. Сервер не доверяет имени и MIME браузера: определяет WAV, MP3, M4A, AAC, WebM, OGG и FLAC по сигнатуре, извлекает длительность из метаданных и нормализует файл. Принимаются записи 0,5–60 секунд до 4 МиБ. Для PCM/WAV рассчитывается RMS и слишком тихий сигнал отклоняется до передачи провайдеру; для любого формата пустая/несодержательная расшифровка отклоняется после транскрипции. Размер расшифровки ограничен 6000 символами до анализа. Ограничения покрыты unit/contract-тестами.

**Остаток:** для сжатых форматов точный анализ громкости до внешнего запроса потребовал бы серверного декодера наподобие FFmpeg, что заметно увеличит Vercel bundle. Текущий безопасный компромисс — сигнатура и длительность до отправки плюс обязательная проверка содержательной расшифровки после неё.
...
| `npm run test` | 38 unit/contract-тестов проходят |
...
Поведение этапов не переписывалось: выполнено безопасное разделение по функциональным границам. Контрактный тест ограничивает координатор 300 строками, а каждый файл этапов — 400 строками, чтобы монолит не вернулся. Lint, TypeScript, 38 unit/contract-тестов и production build проходят.
...
| 12 | AI timeout/cancel и безопасный multipart | Исправлено в коде: timeout, размер, сигнатура, длительность, тишина и расшифровка проверяются | P1 | Убирает зависания и лишнюю нагрузку | Средняя |
...
Локальные lint, TypeScript, 38 unit/contract-тестов и production build проходят. Это необходимое, но недостаточное условие для публичного запуска.
