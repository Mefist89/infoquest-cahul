...
| P1.9 Quality gate | Частично | Локально lint, TypeScript, 33 unit/contract-теста и build проходят; 3 публичных Playwright smoke-теста найдены. Последний GitHub Actions красный из-за рассинхронизации lock-файла при `npm ci`; авторизованные E2E требуют `E2E_AUTH_STATE_B64` |
...
| Блок «Как проходит расследование» | Исправлено 13 августа 2026 | Контейнер расширен до 80 rem, карточки получили увеличенную высоту и внутренние отступы; номер этапа больше не перекрывает заголовок, описание читается свободно |
| Слайдер «Дело: Тень» | Исправлено 13 августа 2026 | Между мини-заданием и слайдером добавлен заметный вертикальный отступ; контейнер дел расширен до тех же 80 rem, что и блок с этапами расследования |
...
| `npm run test` | 33 unit/contract-теста проходят |
...
Поведение этапов не переписывалось: выполнено безопасное разделение по функциональным границам. Контрактный тест ограничивает координатор 300 строками, а каждый файл этапов — 400 строками, чтобы монолит не вернулся. Lint, TypeScript, 33 unit/contract-теста и production build проходят.
...
Локальные lint, TypeScript, 33 unit/contract-теста и production build проходят. Это необходимое, но недостаточное условие для публичного запуска.
