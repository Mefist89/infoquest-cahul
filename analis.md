...
| P1.3 Реальный прогресс на главной | Частично | XP, щит и бейджи читаются из Supabase; основной CTA и для вошедшего пользователя всё ещё ведёт на `/login` |
...
| P1.9 Quality gate | Частично | Локально lint, TypeScript, 31 unit/contract-тест и build проходят; 3 публичных Playwright smoke-теста найдены. Последний GitHub Actions красный из-за рассинхронизации lock-файла при `npm ci`; авторизованные E2E требуют `E2E_AUTH_STATE_B64` |
...
| Supabase и профиль | Полноценный пользовательский dashboard | Адаптивная навигация, текущая миссия, 8 модулей, 8 наград, 64 этапа, статусы и 800 XP синхронизированы с Supabase |
| Админ-панель | Полноценный MVP-dashboard | Адаптивная боковая навигация, реальные сводные показатели и графики, восемь модулей, AI-бюджет, пять ролей и IP/account-блокировки связаны с Supabase |
...
| `npm run test` | 31 unit/contract-тест проходит |
...
Поведение этапов не переписывалось: выполнено безопасное разделение по функциональным границам. Контрактный тест ограничивает координатор 300 строками, а каждый файл этапов — 400 строками, чтобы монолит не вернулся. Lint, TypeScript, 31 unit/contract-тест и production build проходят.
...
**Частично исправлено:** админка получила отдельный адаптивный dashboard: сводные карточки, шестимесячную активность регистраций, распределение ролей, общий прогресс, прохождение каждого модуля и состояние AI-бюджета. Все значения рассчитываются из Supabase и единого каталога, управление ролями и блокировками сохранено.
...
Локальные lint, TypeScript, 31 unit/contract-тест и production build проходят. Это необходимое, но недостаточное условие для публичного запуска.
