...
Профиль оформлен как адаптивный пользовательский dashboard с боковой навигацией на широком экране и компактной навигацией на телефоне. Он показывает:
...
4. Supabase RPC резервирует квоту и конкурентный lock.
5. Аудио передаётся OpenAI-совместимому endpoint транскрипции.
6. Вопрос и расшифровка передаются модели анализа.
7. Модель возвращает структурированный JSON по строгой схеме.
8. Сервер проверяет структуру ответа и возвращает результат UI.
9. Lock освобождается в `finally`, включая ошибки и timeout.
...
| `/[locale]/profile` | Авторизованный | Профиль, XP, награды, прогресс модулей |
| `/[locale]/modules/operator-call` | Авторизованный | Первый учебный модуль |
| `/[locale]/ai-help` | Разрешённая роль | AI-помощник Chrono |
| `/[locale]/admin` | Администратор | Пользователи, роли, прогресс, AI-бюджет, блокировки |
...
- укрепить multipart-проверки по сигнатуре и длительности аудио;
